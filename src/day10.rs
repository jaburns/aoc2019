...
fn asteroid_is_visible_from(map: &Vec<Vec<bool>>, rock_x: i32, rock_y: i32, from_x: i32, from_y: i32) -> bool {
    if !map[rock_y as usize][rock_x as usize] { return false; }
    if rock_x == from_x && rock_y == from_y { return false; }
    ...
    loop {
        sx += dx;
        sy += dy;
        if sx == from_x && sy == from_y { return true; }
        if map[sy as usize][sx as usize] { return false; }
    }
}
...
fn blast_visible_asteroids(map: &mut Vec<Vec<bool>>, x: i32, y: i32) -> Vec<(i32,i32)> {
    let mut targets_with_angles: Vec<((i32,i32),f32)> = find_visible_asteroids(map, x, y).iter()
...
pub fn main() {
    let mut map: Vec<Vec<bool>> = std::fs::read_to_string("data/day10.txt").unwrap()
        .lines()
